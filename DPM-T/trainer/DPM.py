...
class DPM(TrainerX):  # need to be trained
    ...
    def train(self, train_loader, id_loader, ood_loader_list):  # , start_epoch, max_epoch):
        ...
        for self.epoch in range(self.start_epoch, self.max_epoch):
            ...
            if self.epoch > 1:
                _ = self.test_ood(train_loader, id_loader, ood_loader_list)
    ...
    def test_ood(self, train_loader, id_loader, ood_loader_list, method='maxlogits'):
        ...
        save_dic['id'] = in_fea2.tolist()

        ood_name = ['cifar10','ImageNetr','LSUN']
        mean_auroc = 0.0
        for i in range(len(ood_name)):
            ...
            kl_id_norm = np.min(in_kl, axis=1)
            kl_ood_norm = np.min(out_kl, axis=1)                                                                         
            x_max = max(in_kl.max(), out_kl.max())
            x_min =  min(in_kl.min(), out_kl.min())
            ...
            bestscore = -10
            best_rec = []
            for beta in range(0, 200, 1):  # in_score + 1.5*
                beta = beta / 10 - 10
                id_score = beta * (-kl_id_norm) + (-np.max(in_fea2, axis=1))
                ood_score = beta * (-kl_ood_norm) + (-np.max(out_fea2, axis=1))
        ...
        if type(self.class_sim_mean) == type(None):
            ...
            with torch.no_grad():
                class_sim_mean = np.array(
                    [total_logi[(labels == i)].mean(axis=0) for i in range(100)])
