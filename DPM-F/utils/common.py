...
def get_test_labels(args, loader = None):
    ...
    elif args.in_dataset in ['CUB100k']:
        test_labels = loader.dataset.classes
    elif args.in_dataset in ['CUB']:
        test_label = loader.dataset.classes
        test_labels = []
        for item in test_label:
            item =  item.replace(" ", "_")
            test_labels.append(item)
    elif args.in_dataset in ['CUB150k']:
        test_labels = loader.dataset.classes
        test_labels = [item.split('.')[1] for item in test_labels]
    elif args.in_dataset in ['AWA40k']:
        test_labels = loader.dataset.classes
    elif args.in_dataset in ['LADV40k']:
        test_labels = loader.dataset.classes
        test_labels = [item.split('_')[1] for item in test_labels]
    elif args.in_dataset in ['LADA40k']:
        test_labels = loader.dataset.classes
        test_labels = [item.split('_')[1] for item in test_labels]
    elif args.in_dataset in ['LADHA']:
        test_labels = loader.dataset.classes
        test_labels = [item.split('_')[1] for item in test_labels]
    elif args.in_dataset in ['LADHV']:
        test_labels = loader.dataset.classes
        test_labels = [item.split('_')[1] for item in test_labels]
    elif args.in_dataset in ['LADHV']:
        test_labels = loader.dataset.classes
        test_labels = [item.split('_')[1] for item in test_labels]
    elif args.in_dataset in ['FGVC-Aircraft']:
        test_labels = loader.dataset.classes
    elif args.in_dataset in ['Scar']:
        test_labels = loader.dataset.classes
    elif args.in_dataset in ['Cal']:
        test_labels = loader.dataset.classes
    return test_labels
