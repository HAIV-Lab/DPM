...
def get_ood_scores_CALIP(args, model, loader, text_f_yes, softmax=True):
    # model = net
    to_np = lambda x: x.data.cpu().numpy()
    concat = lambda x: np.concatenate(x, axis=0)
    total_features = []
    ...
    with torch.no_grad():
        for batch_idx, (images, label) in enumerate(tqdm_object):
            ...
            # features /= image_features.norm(dim=-1, keepdim=True)
    ...
    img_spatial_feat = total_features[:, 1: , :].cpu()
    img_spatial_feat = img_spatial_feat.permute(0, 2, 1)
    logits =  100*img_global_feat.float() @ feat_t.float().cpu().permute(1, 0)
    # logits = F.softmax(logits / args.T, dim=1)
    # logits = to_np(logits)
    # logits = [np.max(item) for item in logits]
    return logits

def get_and_print_results(args, in_score, out_score):
    '''
    ...
    return measures[2],measures[0],measures[1]
